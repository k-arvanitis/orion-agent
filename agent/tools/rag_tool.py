...
Return format:
  JSON string:
  {"answer": "<text for LLM>", "chunks": [{"source", "heading", "content"}]}
...
def search_policies(query: str) -> str:
    ...
    try:
        ...
        return json.dumps(
            {
                "answer": (
                    "Policy search is temporarily unavailable. Try the database "
                    "for order information, or ask the customer to contact support."
                ),
                "chunks": [],
            }
        )
    ...
    if not result.points:
        ...
        return json.dumps(
            {"answer": "No relevant policy information found.", "chunks": []}
        )
